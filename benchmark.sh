...
BASE_URL="http://historical-prices-deploy-configuration-app:8080/api"
HEALTH_ENDPOINT="$BASE_URL/products/1/prices"
...
for i in {1..30}; do
  ...
  if [[ "$STATUS" == "200" || "$STATUS" == "404" || "$STATUS" == "405" ]]; then
    echo "✅ API disponible (HTTP $STATUS)"
    break
  fi
  echo "Esperando API... intento $i (respuesta $STATUS)"
  sleep 5
done
...
curl -X POST "$BASE_URL/products/$PRODUCT_ID/prices" \
  -H "Content-Type: application/json" \
  -d '{"value":99.99,"initDate":"2024-01-01","endDate":"2024-06-30","currency":"EUR"}'
echo -e "\n"
...
curl -X POST "$BASE_URL/products/$PRODUCT_ID/prices" \
  -H "Content-Type: application/json" \
  -d '{"value":129.99,"initDate":"2024-07-01","endDate":"2024-12-31","currency":"EUR"}'
echo -e "\n"
...
curl -X POST "$BASE_URL/products/$PRODUCT_ID/prices" \
  -H "Content-Type: application/json" \
  -d '{"value":199.99,"initDate":"2025-01-01","endDate":null,"currency":"EUR"}'
echo -e "\n"
